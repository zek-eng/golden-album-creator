...
const THEMES: Record<PosterTheme, ThemePalette> = {
  royal: {
    base: "#0a0402",
    grad: [
      { offset: "0%",  color: "#a8642c" },
      { offset: "22%", color: "#7a3f1a" },
      { offset: "55%", color: "#3a1c0c" },
      { offset: "85%", color: "#1a0a04" },
      { offset: "100%", color: "#0a0402" },
    ],
    warmthTop: "#5a2a10",
    warmthBottom: "#000000",
    glow: { inner: "#ffce85", mid: "#c9772e" },
    floor: { inner: "#ffd8a0", mid: "#a96a2e" },
    metal: [
      { offset: "0%",   color: "#fff1c8" },
      { offset: "25%",  color: "#f3d28a" },
      { offset: "50%",  color: "#caa05a" },
      { offset: "75%",  color: "#8a5a26" },
      { offset: "100%", color: "#efd2a0" },
    ],
    metalLine: "#f3d28a",
    glassEdge: [
      { offset: "0%",   color: "#fff1c8", opacity: 0.9 },
      { offset: "50%",  color: "#caa05a", opacity: 0.55 },
      { offset: "100%", color: "#3a1c0c", opacity: 0.7 },
    ],
    goldGlowMatrix: "1 0 0 0 0.95   0 1 0 0 0.78   0 0 1 0 0.42   0 0 0 0.55 0",
    frame: [
      { offset: "0%",  color: "#caa05a", opacity: 0.55 },
      { offset: "50%", color: "#fff1c8", opacity: 0.95 },
      { offset: "100%", color: "#caa05a", opacity: 0.55 },
    ],
    textSoft: "#d4b07c",
    natureOverlay: "#0a0402",
  },
  ocean: {
    base: "#02080f",
    grad: [
...
export function buildPosterSVG(data: PosterData): string {
  ...
  const warmthOpacity = hasNature ? 0.34 : 1;
  const ambientOpacity = hasNature ? 0.44 : 1;
...
function renderNatureBackdrop(data: PosterData, theme: ThemePalette): string {
  const opacity = Math.min(1, Math.max(0.35, data.bgOpacity ?? 0.82));
  const overlay = Math.min(0.55, Math.max(0, data.bgOverlay ?? 0.2));
  const scale = Math.min(1.6, Math.max(1, data.bgScale ?? 1.1));
  const clearLayerOpacity = Math.min(0.2, opacity * 0.26);
